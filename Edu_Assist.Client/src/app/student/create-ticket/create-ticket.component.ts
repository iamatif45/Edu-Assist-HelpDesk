...
export class CreateTicketComponent implements OnInit {
  ...
  // Custom validator for checking at least 10 words in description
  ...
  validateTitle(title: string): boolean {
    ...
    // Check if title is just a sequence of numbers or if it contains any special characters
  ...
  validateDescription(description: string): boolean {
    const regex = /^[A-Za-z0-9\s,.-]+$/;  // Allow letters, numbers, spaces, commas, periods, and hyphens
    const wordCount = description ? description.split(/\s+/).filter(Boolean).length : 0;

    // Check if the description is valid and contains at least 10 words
    if (wordCount < 10 || !regex.test(description) || /\d{2,}/.test(description)) {
      this.errorMessage = 'Description must contain at least 10 words and can only include valid characters (letters, numbers, spaces, commas, periods, and hyphens). It cannot contain sequences of numbers or special characters.';
      return false;
    }
    return true;
  }
  ...
  onSubmit(form: NgForm): void {
    ...
    if (form.invalid) {
      this.errorMessage = 'Please fill in all required fields correctly, including the description with at least 10 words.';
    ...
    const newTicket: Ticket = {
      ...this.ticket, // Spread the ticket object to include the values from the form
